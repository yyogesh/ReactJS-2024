...
function App() {
  ...
  const renderOption = (option: string, index: number) => {
    ...
    return <button key={option}
      ...
      className={`${bgColor} border-2 border-gray-300 rounded-lg p-4 mb-2 w-full text-left hover:bg-gray-100 transition-colors ${gameState === 'playing' ? 'cursor-pointer' : 'cursor-default'
        }`}>
      {option}
